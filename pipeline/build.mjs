...
// Kyiv: ONE feed from the city's open-data portal covering the surface
// network — 96 bus lines, 43 trolleybus lines and 17 tram lines, the last of
// them including the two Shvydkisnyi Tramvai light-rail routes. The metro is
// NOT in it: Kyivskyi Metropoliten publishes no GTFS at all.
// Usage: node pipeline/build.mjs [--all | lines...] [--tram all|1,14]
...
// Exact-match repairs for poles a feed names badly. Kyiv needs none: of 1 445
// stop names only 58 shout, and each is an acronym the city itself shouts
// (АЗС the filling station, ТРЕД the tram depot, ФАП the village surgery).
const NAME_FIX = {};
...
// ONE feed, TWO cfgs.
//
// The Kyiv data portal publishes the surface network as one GTFS with shapes:
// route_type 3 bus, 11 trolleybus, 0 tram.
//
// LINE KEYS. All three modes number from 1, so "11" is a bus AND a trolleybus
// AND a tram in this city. The trolleybuses ride the road graph, which means
// bus 11 and trolleybus 11 would land on the same key in the same cfg and be
// welded into one line. They get a Тб prefix in the KEY and the trams a Тм —
// Sofia's rule, for exactly Sofia's reason — and LBL prints the bare number
// the stop flag shows. Nothing prints twice on a street: the colour says which
// mode it is, navy, green or red.
//
// The feed fills no trip_headsign anywhere, so directions would go nameless;
// they are named after the last stop of the drawn pattern instead (the same
// repair the Swedish feeds needed).
const LBL = new Map();
...
const busKey = (sn, r) => {
  const s0 = norm(sn);
  if (!s0) return null;
  if (r.route_type !== '11') return s0;
  const k = 'Тб' + s0;
  LBL.set(k, s0);          // the prefix is the key's, never the street's
  return k;
};
const tramKey = (sn) => {
  const s0 = norm(sn);
...
const MODES = [{
  ...
  // the city and its right-bank and left-bank suburbs — 3 × 3 tiles
  osmFiles: Array.from({ length: 9 }, (_, i) => `data/osm/tiles/t${i + 1}.json`),
  color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['62']),
  feeds: [
    { tag: 'kyiv', dir: 'data/gtfs', routeTypes: ['3', '11'], mapKey: busKey },
  ],
}];
const tramAll = tramLines.length === 1 && tramLines[0] === 'all';
if (tramAll || tramSel.length) MODES.push({
  mode: 'tram', label: 'trams', osmFile: 'data/osm/kyiv-rail.json',
  graphMode: 'tram', railKeep: new Set(['tram', 'light_rail']),
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : tramSel.map((l) => (/^Тм/.test(l) ? l : 'Тм' + norm(l))),
  feeds: [
    { tag: 'kyiv', dir: 'data/gtfs', routeTypes: ['0'], mapKey: tramKey },
  ],
});
